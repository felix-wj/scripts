...
================Loon==============
[Script]
cron "0 0 0 1 * ?" script-path=jd_jdzz.js,tag=京东赚赚
...
let cookiesArr = [], cookie = '', message = '', allMessage = ''  ,prizeMessage='';
if ($.isNode()) {
...
function getPrize(){
  doPrizeTask({"prizeId":5})
}
function doPrizeTask(body, func = "exchangePrize") {
    ...
    $.get(taskUrl(func, body), async (err, resp, data) => {
      try {
        if (err) {
          ...
          console.log(data)
          prizeMessage += data;
          notify.sendNotify($.name, prizeMessage);
        }
      } catch (e) {
        $.logErr(e, resp)
      } finally {
...
function taskUrl(functionId, body = {}) {
  return {
    url: `${JD_API_HOST}?functionIdTest=${functionId}&functionId=${functionId}&body=${escape(JSON.stringify(body))}&client=wh5&clientVersion=9.1.0`,
    headers: {
      'Cookie': cookie,
      'Host': 'api.m.jd.com',
