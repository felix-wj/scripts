//微信打开：http://www.qgftzss.cn?masterid=oFFbU5hJN8RbyvTMmkcdO-JB5n_A
//export cdbcookie="unionid=123&token=456@unionid=123&token=456"
//应该是一天一次
...
let userCookie = ($.isNode() ? process.env.cdbcookie : $.getdata('cdbcookie')) || '';
...
class UserInfo {
    ...
    async getuseinfo() {
        try {
            let url = `http://www.udnudw.cn/user/getuseinfo`
    ...
    async activeone() {
        try {
            let url = `http://www.udnudw.cn/user/addmyorder0`
            let body = JSON.stringify({"unionid":this.param.unionid})
            let unionid =`${this.param.unionid}`
            let token =`${this.param.token}`
            let urlObject = populateUrlObject(url,token,unionid,body)
   ...
   async pushcash() {
        try {
            let url = `http://www.udnudw.cn/trade/pushcash`
            let body = JSON.stringify({"unionid":this.param.unionid,"money": this.money})
            let unionid =`${this.param.unionid}`
            let token =`${this.param.token}`
            let urlObject = populateUrlObject(url,token,unionid,body)
            await httpRequest('post',urlObject)
...
function populateUrlObject(url,token,unionid,body=''){
    ...
    let urlObject = {
        ...
        headers: {
            ...
            'Referer': 'http://www.kvyoff.cn/',
            'X-Requested-With': 'XMLHttpRequest',
            'Origin': 'http://www.kvyoff.cn',
            'token': token,
            'unionid': unionid,
